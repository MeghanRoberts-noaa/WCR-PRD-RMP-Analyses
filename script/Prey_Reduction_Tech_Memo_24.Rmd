...
data<-read_csv(here("data","USALLValid_TechMemo.csv")) #old data
# write_csv(data, "tech_memo_data_nonshp.csv")
percent_reduction_current<-read_csv(here("data", "tech_memo", "SRKW_reduction_percent_RMP.csv")) #2019 PST+RMP scenario
```

#cleaning up the input data to make a nice map
```{r}
### getting the df in a workable format
#convert years to numeric
percent_reduction_current$Year<-as.numeric(percent_reduction_current$Year)
#remove percent sign
...
#rename Cali
percent_reduction_current<-percent_reduction_current %>% rename("CA"="Cali")

# Apply the function to multiple columns
percent_reduction_current <- percent_reduction_current  %>%
  mutate_at(vars(Salish, OR, NOF, CA, SWWCVI), list(~substitute_values(.)))

#filter for only Zero_all scenario
percent_reduction_current<-percent_reduction_current %>% filter(Scenario %in% "ZeroALL")

###group by and summarize
percent_reduction_avg<- percent_reduction_current %>%
  group_by(TimeStep) %>%
  summarise(
    ...
    avg_percent_reduction_CA = round(mean(CA, na.rm = TRUE),2),
    avg_percent_reduction_SWWCVI = round(mean(SWWCVI, na.rm = TRUE),2),
    .groups = 'drop'
  )

percent_reduction_10yr<-percent_reduction_current %>% filter(Year>2009)

percent_reduction_avg_10yr <- percent_reduction_10yr %>%
  group_by(TimeStep) %>%
  summarise(
    avg_percent_reduction_Salish = round(mean(Salish, na.rm = TRUE),2),
    avg_percent_reduction_OR = round(mean(OR, na.rm = TRUE),2),
...
#now we need to put the data in tidyformat
percent_reduction_avg_longer<-pivot_longer(data= percent_reduction_avg, cols = !c(TimeStep), names_to = "region", values_to = "avg_per_reduction" ) %>% rename("timestep"="TimeStep")
percent_reduction_avg_longer$region<-str_replace_all(percent_reduction_avg_longer$region, "avg_percent_reduction_", "")
percent_reduction_avg_longer$timestep<-str_replace_all(percent_reduction_avg_longer$timestep, "_", "-")

percent_reduction_avg_longer_10<-pivot_longer(data= percent_reduction_avg_10yr, cols = !c(TimeStep), names_to = "region", values_to = "10yr_per_reduction" ) %>% rename("timestep"="TimeStep")
percent_reduction_avg_longer_10$region<-str_replace_all(percent_reduction_avg_longer_10$region, "avg_percent_reduction_", "")
percent_reduction_avg_longer_10$timestep<-str_replace_all(percent_reduction_avg_longer_10$timestep, "_", "-")

data_2019<-full_join(percent_reduction_avg_longer_10, percent_reduction_avg_longer)

#join the data
data_spatial<-left_join(regions_sp, data_2019) #changed this out for the 2019 data for the tech memo
...
#adding code to show high SRKW presence
oct_apr$SRKW_risk<-"Low"
oct_apr$SRKW_risk[oct_apr$region %in% c("NOF", "CA", "OR")] <- 'High'
oct_apr$SRKW_risk <- factor(oct_apr$SRKW_risk, levels = rev(unique(oct_apr$SRKW_risk))) #sets the order of the characters
may_jun$SRKW_risk<-"Low"
may_jun$SRKW_risk[may_jun$region %in% c("SWWCVI")] <- 'High'
may_jun$SRKW_risk <- factor(may_jun$SRKW_risk, levels = rev(unique(may_jun$SRKW_risk))) #sets the order of the characters
jul_sep$SRKW_risk<-"Low"
jul_sep$SRKW_risk[jul_sep$region %in% c("SWWCVI", "Salish")] <- 'High'
jul_sep$SRKW_risk <- factor(jul_sep$SRKW_risk, levels = rev(unique(jul_sep$SRKW_risk))) #sets the order of the characters
...
oct_apr_map <- ggplot() +
  geom_sf(data = US, aes()) +
  geom_sf(data = CAN, aes()) +
  geom_sf_pattern(data = oct_apr, aes(fill = avg_per_reduction, pattern=SRKW_risk), 
                  pattern_fill='#e5975a', 
                  pattern_density=0.07, 
                  pattern_color="#e5975a"
                  ) +
  scale_pattern_manual(values = c("Low"="none", "High"="crosshatch"))+
  ...
  geom_sf_text(data = oct_apr, aes(label = paste(region, " = \n", avg_per_reduction, "%")),
               color = "black", size=5, fun.geometry = sf::st_centroid, 
               nudge_x=oct_apr$x_nudge, nudge_y = oct_apr$y_nudge)+
...
may_jun_map <- ggplot() +
  geom_sf(data = US, aes()) +
  geom_sf(data = CAN, aes()) +
  geom_sf_pattern(data = may_jun, aes(fill = avg_per_reduction, pattern=SRKW_risk), 
                  pattern_fill='#e5975a', 
                  pattern_density=0.07, 
                  pattern_color="#e5975a") +
  scale_pattern_manual(values = c("Low"="none", "High"="crosshatch"))+
...
jul_sep_map <- ggplot() +
  geom_sf(data = US, aes()) +
  geom_sf(data = CAN, aes()) +
  geom_sf_pattern(data = jul_sep, aes(fill = avg_per_reduction, pattern=SRKW_risk), 
                  pattern_fill='#e5975a', 
                  pattern_density=0.07, 
                  pattern_color="#e5975a") +
  scale_pattern_manual(values = c("Low"="none", "High"="crosshatch"))+
  ...
  geom_sf_text(data = jul_sep, aes(label = paste(region, " = \n", avg_per_reduction, "%")),
               color = "black", size=5, fun.geometry = sf::st_centroid, 
               nudge_x=oct_apr$x_nudge, nudge_y = oct_apr$y_nudge)+
  labs(x="", y="", fill="Average Percent\nReduction", pattern="SRKW Presence",
...
legend<-get_legend(jul_sep_map)
jul_sep_map<-jul_sep_map + theme(legend.position = "none")
perc_reduction_map<-plot_grid(oct_apr_map, may_jun_map, jul_sep_map, legend, ncol = 4)

perc_reduction_map
ggsave(here("saved_figures","tech_memo", "regional_per_reduction_tech_memo_flat.jpeg"), plot = perc_reduction_map, width = 14, height = 6, dpi = 300)
```
